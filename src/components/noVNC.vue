...
const props = withDefaults(defineProps<VueVncProps>(), {
  onClipboard: async function(data) {
    try {
      await navigator.clipboard.writeText(data.detail.text);
    } catch (err) {
      const textArea = document.createElement("textarea");
      textArea.value = data.detail.text;
...
const disconnect = () => {
  ...
  try {
    ...
    (Object.keys(eventListeners.value) as (keyof NoVncEvents)[]).forEach(
      (event) => {
        if (typeof event != 'symbol' && eventListeners.value[event]) {
